#!/usr/bin/env node
// Fixture set "chaosmnli" from tasksource/chaos-mnli-ambiguity (formerly
// metaeval/chaos-mnli-ambiguity), the MNLI subset of ChaosNLI (Nie et al., 2020)
// with 100 annotators per item. Criterion: does the
...
import { chaosMnliCounts, huggingFaceRows, pad, rawRoot, seededShuffle, writeFixture } from "./common.mjs";
...
const cases = chosen.map((x, i) => ({
  ...
  softLabel: x.s,
  note: `uid ${x.r.uid}; counts ${JSON.stringify(chaosMnliCounts(x.r))}; majority ${x.r.majority_label}; original 5 labels ${JSON.stringify(x.r.old_labels)}`
}));
...
for (const [premise, xs] of byPremise) {
  ...
  const c = xs.find((x) => x.r.majority_label === "c" && (chaosMnliCounts(x.r).c ?? 0) >= 85);
...
if (pairs.length < 8) {
  ...
  const cs = items.filter((x) => x.r.majority_label === "c" && (chaosMnliCounts(x.r).c ?? 0) >= 90);
