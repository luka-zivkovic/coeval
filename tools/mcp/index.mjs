#!/usr/bin/env node
// rubrist MCP server (stdio) — issue #10.
//
...
// run_gate_check. Configuration is RUBRIST_URL + RUBRIST_API_KEY in the
...
import { z } from "zod";
import { createRubristClient } from "./lib.mjs";

const server = new McpServer({ name: "rubrist", version: "0.1.0" });
...
function tool(name, description, inputShape, invoke) {
  server.registerTool(name, { description, inputSchema: inputShape }, async (args) => {
    try {
      const rubrist = createRubristClient({
        baseUrl: process.env.RUBRIST_URL ?? "",
        apiKey: process.env.RUBRIST_API_KEY ?? ""
      });
      const result = await invoke(rubrist, args ?? {});
      return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
    } catch (error) {
      return {
        isError: true,
        content: [{ type: "text", text: `rubrist: ${error instanceof Error ? error.message : String(error)}` }]
      };
    }
  });
}
...
tool(
  ...
  (rubrist, args) => rubrist.getFindings(args)
);
...
tool(
  ...
  "Project cases with FULL stored inputs/outputs (as ingested and redacted at write time) plus the latest judge and human verdicts — the exact material to re-run a skill patch on. Filterable by effective verdict, metadata.stratum, and creation time.",
  {
    ...
    limit: z.number().int().positive().max(200) /* mirrors V1_CASES_MAX_LIMIT in @rubrist/shared — keep in lockstep */.optional().describe("Max cases to return (default 50, max 200)")
  },
  (rubrist, args) => rubrist.getCases(args)
);
...
tool(
  ...
  (rubrist, args) => rubrist.getGolden(args)
);
...
tool(
  ...
  (rubrist, args) => rubrist.submitRuns(args)
);
...
tool(
  ...
  (rubrist, args) => rubrist.runGateCheck(args)
);
