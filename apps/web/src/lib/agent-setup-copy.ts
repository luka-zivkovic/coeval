...
Offer “Finish setup” or “Refine the Check.” Do not ask me to create a Rubrist connection until I choose Finish setup. Never read .env or credential files, invent a Run, make a human judgment, promote a Golden example, or make a release decision.`;
...
Use the $rubrist-setup skill (or read https://github.com/luka-zivkovic/rubrist/blob/main/plugins/rubrist/skills/rubrist-setup/SKILL.md if it is unavailable). Apply the exact Check proposal I reviewed; if no proposal has been reviewed yet, pause and complete its short context-first preparation flow before using this connection. Use the rubrist-audit transport for ongoing capture and submission. Provide the token through RUBRIST_PAIRING_TOKEN; never write it into setup files or repeat it in output. Submit a first batch only when a real Run is already available. Report the Check as Starter · unvalidated, and stop before human adjudication, Golden promotion, governed activation, or release decisions.
