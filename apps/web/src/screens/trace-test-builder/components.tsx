...
export const EMPTY_FIELDS: ManualTraceTestFields = {
  scenario: "",
  expectedBehavior: "",
  mustDo: "",
  mustAvoid: "",
...
export const HUMAN_PROVENANCE: TraceTestDraftProvenance = {
  origin: "human",
  generatedFields: [],
  generator: null
};

export type EditableDraftTextField = Exclude<keyof ManualTraceTestFields, "checkerKind" | "checkerLabel" | "checkerRationale">;
...
export const MOBILE_FOCUS_CLASS = "max-[760px]:fixed max-[760px]:inset-0 max-[760px]:z-50 max-[760px]:overflow-y-auto max-[760px]:bg-paper max-[760px]:px-4 max-[760px]:pt-4";

export function lastRevision(test: TraceTestDetail): TraceTestRevision {
...
export function restoredJob(revision: TraceTestRevision): Exclude<TraceTestJob, "verdict"> {
  const job = revision.checker.metadata.journeyJob;
  return job === "preserve" ? "preserve" : "response";
}

export function restoredInferredContext(revision: TraceTestRevision): string[] {
...
export function parseIntent(value: string | null): TraceTestIntent | null {
  return value === "prevent" || value === "protect" || value === "make" ? value : null;
}

export function journeyStatus(input: {
  stage: Stage;
...
export function SourceStage(props: SourceStageProps) {
...
export function DraftStage(props: DraftStageProps) {
...
export function ValidationStage(props: ValidationStageProps) {
...
export function LocalState({ title, body }: { title: string; body: string }) {
...
export function UnsavedDialog({ canSave, incompleteDraft, submitting, error, onKeep, onDiscard, onSave }: { canSave: boolean; incompleteDraft: boolean; submitting: boolean; error: string | null; onKeep: () => void; onDiscard: () => void; onSave: () => Promise<void> }) {
