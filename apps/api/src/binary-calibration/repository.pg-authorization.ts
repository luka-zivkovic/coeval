...
import { legacyModelBinding } from "../lib/execution-binding.js";
...
export async function deriveRunIdentity(
  ...
  const modelBinding = legacyModelBinding(skillVersion);
  if (modelBinding === null) {
    throw repoError("unsupported", "sealed calibration v1 requires an explicit temperature and a prompted provider; this binding needs calibration v2 (Batch 8D)");
  }
...
export async function loadAuthorizedRun(
  ...
  const binding = legacyModelBinding({
  ...
  if (binding === null) throw repoError("unsupported", "sealed calibration v1 requires an explicit temperature and a prompted provider");
  return {
    claim: {
      runId: claim.runId,
