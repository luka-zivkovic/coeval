...
run("PgBinaryCalibrationRepository", () => {
  ...
  beforeAll(async () => {
    ...
    await saveResolutionRecord(pool, PROJECT_ID, skillVersionId, await resolvedRecordFor(SEEDED_BINDING));
  ...
  it("pins idempotent identity and permits only one active run per revision/evaluator", async () => {
    ...
    try {
      ...
      await saveResolutionRecord(pool, PROJECT_ID, skillVersionId, await temperatureRejectingRecordFor(opus));
  ...
  it("allows a same-version rerun, rejects a post-test version, and reports later revocation", async () => {
    ...
    expect((await repository.getRecheckTarget(recheckClaim!)).lastUnknownRecheckAt).not.toBeNull();
    ...
    await saveResolutionRecord(pool, PROJECT_ID, later.id, await resolvedRecordFor(SEEDED_BINDING));
