...
test.describe("trace deep links", () => {
  test("an unsupported source shows the friendly invalid-link page", async ({ page }) => {
    ...
    await expect(page.getByText("Coeval can't open this link.")).toBeVisible();
  ...
  test("an unknown Ironside trace explains that it has not been imported yet", async ({ page }) => {
    ...
    await expect(page.getByText("This trace hasn't been imported into Coeval yet.")).toBeVisible();
    await expect(page.getByText("None of your Coeval projects is connected to Ironside project")).toBeVisible();
  });
});
