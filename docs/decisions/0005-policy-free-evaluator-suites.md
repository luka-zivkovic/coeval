...
smuggle release policy into Rubrist. Conversely, leaving every evaluator
unrelated makes it difficult to reproduce which set was applied together.

## Decision

One evaluator measures one named criterion. Rubrist may group evaluators in a
...
Rubrist emits separately verifiable assessment and calibration evidence per
...
Rubrist does not mark suite criteria mandatory, advisory, compensatory, or
